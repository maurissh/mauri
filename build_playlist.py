...
import epg as epg_module  # modulo locale per la guida programmi
...
# EPG (guida programmi). Lo script scarica una guida XMLTV completa, la filtra
# sui canali in playlist e RIALLINEA gli id al nostro tvg-id, salvando un
# epg.xml locale. Cosi' la guida si aggancia anche ai canali presi da Free-TV.
# - EPG_SOURCE_URL: la guida XMLTV completa da cui partire (vuota = niente EPG)
# - EPG_OUTPUT_NAME: il file generato, a cui la playlist puntera' via url-tvg
# - EPG_PUBLIC_URL: l'URL pubblico del file generato nel TUO repo (per l'header).
#   Sostituisci UTENTE/REPO coi tuoi. Se vuoto, l'header usa il nome file relativo.
EPG_SOURCE_URL = "https://tvit.leicaflorianrobert.dev/epg/list.xml"
EPG_OUTPUT_NAME = "epg.xml"
EPG_PUBLIC_URL = "https://raw.githubusercontent.com/maurissh/mauri/refs/heads/main/epg.xml"
...
EPG_OUTPUT_FILE = ROOT / EPG_OUTPUT_NAME
...
def build():
    ...
    # --- Genera l'EPG filtrato e allineato sui canali finali ---
    epg_ok = False
    if EPG_SOURCE_URL:
        print("Genero la guida EPG filtrata sui canali in playlist...")
        stats = epg_module.build_filtered_epg(
            EPG_SOURCE_URL, entries, normalize, lcn_index, EPG_OUTPUT_FILE
        )
        epg_ok = stats is not None

    write_m3u(entries, epg_ok)
...
def write_m3u(entries, epg_ok=False):
    """Scrive la playlist M3U con il tag tvg-chno (numero LCN)."""
    # se l'EPG e' stato generato, l'header punta al nostro file pubblicato
    if epg_ok and EPG_PUBLIC_URL:
        lines = [f'#EXTM3U url-tvg="{EPG_PUBLIC_URL}"']
    elif epg_ok:
        lines = [f'#EXTM3U url-tvg="{EPG_OUTPUT_NAME}"']
    else:
        lines = ['#EXTM3U']
    for r in entries:
